...
function showTab(tab){
  ...
  const data = subjects[currentSubject][tab];
  if(tab==="quiz"){
    quizIndex=0;
    startQuiz();
    ...
    data.forEach(ex=>{
      ...
      if(ex.image) html+=`<img src="${ex.image}" style="max-width:100%;">`;
      html+="</div><hr>";
    });
    ...
    let html="";
    data.forEach(v=>{
      html+=`<div><h3>${v.title}</h3><video src="${v.src}" controls style="max-width:100%;"></video></div><hr>`;
    });
    contentDiv.innerHTML=html;
  } else if(tab==="bonus"){
    let html="";
...
function startQuiz(){
  const quizData = subjects[currentSubject]["quiz"];
...
function answerQuiz(choice){
  const q = subjects[currentSubject]["quiz"][quizIndex];
...
function updateProgress(){
  const total = subjects[currentSubject]["quiz"].length;
